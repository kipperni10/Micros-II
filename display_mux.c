...
uint8_t buffer_display[3] = {0, 0, 0}; 
...
void Converte_Para_Buffer(uint32_t valor) {
    buffer_display[2] = (valor / 100) % 10; 
    buffer_display[1] = (valor / 10) % 10;  
    buffer_display[0] = valor % 10;         
}

void Envia_Dado_Display(uint8_t valor) {
    // Exemplo genérico: mapeará GPIO_PIN_x conforme sua fiação
...
void Atualiza_Interface_Visual() {
    ...
    // 1. Lógica de Navegação (Botão Função) Não-Bloqueante (CORREÇÃO 2)
    ...
    if (HAL_GPIO_ReadPin(PORT_BOTOES, PINO_BOTAO_FUNCAO) == GPIO_PIN_SET) {
        if (!botao_pressionado_anteriormente) {
            // Registra a transição de borda de subida (debounce por máquina de estado)
        ...
        // Reseta o estado apenas quando o utilizador soltar o botão fisicamente
    ...
    switch (tela_selecionada) {
        ...
        case TELA_RPM:
            ...
            break;
        case TELA_HODOMETRO:
            Converte_Para_Buffer(((odometria_pulsos / 2) * 194) / 100000); 
            break;
